...
export const StatefulButton = ({ className, children, status, idleText, loadingText, successText, errorText, ...props }: ButtonProps) => {
  ...
  return (
    ...
    >
      <motion.div layout className="flex items-center gap-2">
        ...
        <CheckIcon />
        <motion.span layout>
          {status === 'loading' ? loadingText : status === 'success' ? successText : status === 'error' ? errorText : idleText || children}
        </motion.span>
      </motion.div>
    </motion.button>
  );
